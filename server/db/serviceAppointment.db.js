...
const getPetIdFromAppointmentdb = async (appointment_id) => {
  const query =
    `SELECT appointment_orders.pet_id
...
const createMedicalRecorddb = async ({ neutered, symptoms, diagnostic }) => {
  ...
  `;
  const { rows } = await pool.query(insertMedicalRecordQuery, [neutered, symptoms, diagnostic]);
  return rows[0];
};
...
const updateAppointmentWithMedicalRecordIddb = async (medicalRecordId, appointment_id) => {
  try {
...
const createPrescriptiondb = async ({ medicine, dosage, note, medical_record_id }) => {
  const query = `
...
const getMedicalRecordsByAppointmentIdDb = async (appointment_id) => {
  try {
    ...
    throw new Error('Error in getAllMedicalRecordsByAppointmentIdDb: ' + error.message);
  }
};
...
const updateMedicalRecordDb = async ({ medical_record_id, neutered, symptoms, diagnostic }) => {
  const query = `
...
const updatePrescriptiondb = async (newPrescriptions) => {
  try {
    const medical_record_id = newPrescriptions.length > 0 ? newPrescriptions[0].medical_record_id : null;
    const selectQuery = `
      SELECT *
    ...
    for (let id of preItemIds) {
      ...
      if (prescription) {
        ...
        console.error(`Prescription item with ID ${id} not found in newPrescriptions`);
      }
    }
