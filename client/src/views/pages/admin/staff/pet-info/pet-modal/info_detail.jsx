                            ...
                            <div className='m-10 flex flex-row gap-20'>
                                ...
                                <div className='flex flex-row justify-around w-full'>
                                    <div className='pet-info-detail text-xl flex flex-col gap-14' style={{ flex: 1, paddingRight: 20 }}>
                                        <p><strong>ID thú cưng:</strong> {selectedPet.pet_id}</p>
                                        <p><strong>Tên thú cưng:</strong> {selectedPet.fullname}</p>
                                        <p><strong>Giới tính:</strong> {selectedPet.sex.toLowerCase()}</p>
                                        <p><strong>Tuổi:</strong> {selectedPet.age}</p>
                                        <p><strong>Cân nặng:</strong> {selectedPet.weight}</p>
                                        <p><strong>Chủng loại:</strong> {selectedPet.species}</p>
                                    </div>
                                    <div className='pet-info-detail text-xl flex flex-col gap-14' style={{ flex: 1 }}>
                                        <p><strong>ID khách hàng:</strong> {selectedPet.user_id}</p>
                                        <p><strong>Sức khỏe:</strong> {selectedPet.health}</p>
                                        <p><strong>Mô tả:</strong> {selectedPet.describe}</p>
                                        <Space className='flex flex-col gap-14' direction='vertical' size="middle">
                                            <a className='text-blue-500 text-xl' onClick={showMedicalRecord}>Xem hồ sơ bệnh án</a>
                                            <a className='text-blue-500 text-xl' onClick={showDietPlan}>Xem chế độ ăn</a>
                                        </Space>
                                    </div>
                                </div>
